...
export class NsisCompatUpdater {
  public onDownloadProgress: Event<IStreamProgress> = new Event(
    'downloadProgress'
  )
  ...
  public async downloadUpdate(version: string): Promise<string> {
    ...
    const { url, hash } = (() => {
      const updater = updaters.filter(
        updater =>
          updater.fromVersion == this.currentVersion &&
          updater.arch == this.currentArch
      )[0]
      ...
      const installer = installers.filter(
        installer => installer.arch == this.currentArch
      )[0]
  ...
  protected async download(
    url: string,
    path: string,
    onProgress?: (state: IStreamProgress) => void
  ) {
    ...
    await new Promise((resolve, reject) => {
      stream.pipe(progress).pipe(createWriteStream(path)).on('finish', resolve)
    })
  }
}
